...
def load_pretrained(
    ...
    # config_kwargs["use_auth_token"] = True#使用自己的access token
...
def prepare_data(
    ...
    for dataset_info in data_args.dataset_list:
        ...
        if dataset_info.load_from == "hf_hub":
            ...
            #     data_files=data_file,
            #     cache_dir=model_args.cache_dir,
            #     keep_in_memory=True,  # 不用磁盘缓存
            #     download_mode="force_redownload"  # 强制重新读取文件
            # )
        else:
            raise NotImplementedError

        dataset = raw_datasets[data_args.split]#HuggingFace Datasets 自动把单文件映射成 train
...
def preprocess_data(
    ...
    if stage == "sft":
        ...
        preprocess_function = preprocess_function_train#无论训练还是评估都使用train，修复这个bugValueError: Expected input batch_size (504) to match target batch_size (136).
    elif stage == "rwd":
